...
OUT_FILE = OUT_DIR / "pfsense_logs.csv"
...
def gen_normal(n=3000):
    """Tráfego normal: poucos destinos, portas comuns, ações mistas."""
    rows = []
    for i in range(n):
...
def gen_port_scan(n=30):
    """Port Scan: 1 IP varre muitas portas em curto intervalo."""
    rows = []
    for _ in range(n):
...
def gen_brute_force(n=20):
    """Brute Force: muitas conexões na mesma porta (SSH/FTP)."""
    rows = []
    for _ in range(n):
...
def gen_ddos(n=10):
    """DDoS simplificado: alto volume de conexões em janela curta."""
    rows = []
    for _ in range(n):
